...
RelayMenu::RelayMenu(RelaySharedState& state)
    : m_state(state), m_menu_state(MENU_MAIN), m_rx_index(0)
{
    ...
    switch (m_menu_state) {
        ...
        case MENU_MAIN:
            switch (c) {
                ...
                case '6': m_menu_state = MENU_RESET; show_reset_menu(); break;
        ...
        /* ── Standard selection ─────────────────────────── */
        case MENU_STANDARD:
            if      (c == '1') { *m_state.std = STD_IEC;  Serial.print(F("\r\nSet to IEC."));  }
            else if (c == '2') { *m_state.std = STD_IEEE; Serial.print(F("\r\nSet to IEEE.")); }
        ...
        case MENU_RESET:
            if (c == 'Y' || c == 'y') {
                *m_state.latched = false;  /* reset the latch */
...
void RelayMenu::show_main_menu() {
    ...
    Serial.print(F("\r\n TMS/TDS  : "));
    Serial.print(*m_state.tms, 2);
    Serial.print(F("\r\n Pickup   : "));
    Serial.print(*m_state.pickup, 2);
    Serial.print(F(" A\r\n Inst M   : "));
    Serial.print(*m_state.inst_m, 2);
    Serial.print(F("\r\n--------------------------\r\n"
                   " 1) Change standard\r\n"
                   " 2) Change curve\r\n"
                   " 3) Change TMS/TDS\r\n"
